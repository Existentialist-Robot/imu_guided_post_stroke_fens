/*
 * Firewall Allowances (all Domain and Private networks) - still need to whittle down to find least provisions needed
 *  esp32-inbound | UDP, Ports 3000-3010 - for python commands
 *  esp-tcp-ftp | UDP all ports for this board's IP - 192.168.0.129
 *  FileZilla Port | TCP 14148
 *  FileZilla-executable C:\Program Files\FileZilla\filezilla-server.exe Server executable through port 20-21 over tcp
 *  
 */
...
#define JSON false
#define DEBUG false // currently setup for debuging the main data acquisition loop
...
//      dataMessage = String(lastUpdateTime);
    } else {
...
//        Serial.println(String("DEBUG 5, mainState: ") + String(mainState));
        ...
        if (fileClosed && !fileTransfered){
          ftp.OpenConnection();
          
          // Make a new directory, according to pariticpant number, if not already made
          int len = 4;
          char dirName[len+1];
          ...
          // Make a new sub directory, for imu specific data
          char newSubDir[24];
          
          strcpy(newSubDir, dirName);
          strcat(newSubDir, "_");
          strcat(newSubDir, imuIndex);

          ftp.InitFile("Type A");
          ftp.MakeDir(newSubDir);
          ftp.ChangeWorkDir(newSubDir);
...
//          ftp.CloseFile();
          ...
          ftp.InitFile("Type A");
          readAndSendBigBinFile(SD, fileName, ftp);                 // 272,546
