...
def find_csv_filenames( path_to_dir, suffix=suffix[1] ):
...
#%%
df.columns = ['sampleCount','trigNum','sensorReadTime','ax','ay','az','gx','gy','gz','mx','my','mz'] 
...
df['sensorReadTime'] -= df['sensorReadTime'][0] # standardize times
print(df)

#%%

time_np = np.array(df['sensorReadTime'])
time_offset = []
for x in range(len(time_np)-1):
    time_offset.append(time_np[x + 1] - time_np[x])
    
#%% Sampling

sampling_rate = 100
x_range = np.arange(0,1/sampling_rate*(len(df)),1/sampling_rate)

#%% Basic Plots - acceleration

plt.figure(0)
plt.plot(x_range,df['ax']) 
plt.plot(x_range,df['ay']) 
plt.plot(x_range, df['az']) 
...
plt.figure(0)
plt.plot(x_range,df['gx']) 
plt.plot(x_range,df['gy']) 
plt.plot(x_range, df['gz']) 
...
plt.figure(0)
plt.plot(x_range,df['mx']) 
plt.plot(x_range,df['my']) 
plt.plot(x_range, df['mz']) 
plt.title('Field Strength vs. Time')
plt.xlabel("Time (s)")
...
ax.scatter(df['mx'], df['my'], color='r')
ax.scatter(df['my'], df['mz'], color='g')
ax.scatter(df['mz'], df['mx'], color='b')

#%% Basic Plots - mag #3
fig = plt.figure()
ax = fig.add_subplot(projection='3d')
ax.scatter(df['mx'],df['my'],df['mz'],s=2)
plt.show()
...
#%% Center Detrend - Median (Accel + Gyro) - just subracting median
accel_cal = [ (max(df['ax']) + min(df['ax'])) / 2, (max(df['ay']) + min(df['ay'])) / 2, (max(df['az']) + min(df['az'])) / 2]
df['ax_detrend'] = [x - accel_cal[0] for x in df['ax']]
df['ay_detrend'] = [y - accel_cal[1] for y in df['ay']]
df['az_detrend'] = [z - accel_cal[2] for z in df['az']]

gyro_cal = [ (max(df['gx']) + min(df['gx'])) / 2, (max(df['gy']) + min(df['gy'])) / 2, (max(df['gz']) + min(df['gz'])) / 2]
df['gx_detrend'] = [x - gyro_cal[0] for x in df['gx']]
df['gy_detrend'] = [y - gyro_cal[1] for y in df['gy']]
df['gz_detrend'] = [z - gyro_cal[2] for z in df['gz']]


#%% Center Detrend - Mean - (Accel + Gyro) - just subracting median
accel_cal = [mean(df['ax']), mean(df['ay']), mean(df['az'])]
df['ax_detrend'] = [x - accel_cal[0] for x in df['ax']]
df['ay_detrend'] = [y - accel_cal[1] for y in df['ay']]
df['az_detrend'] = [z - accel_cal[2] for z in df['az']]

gyro_cal = [mean(df['gx']), mean(df['gy']), mean(df['gz'])]
df['gx_detrend'] = [x - gyro_cal[0] for x in df['gx']]
df['gy_detrend'] = [y - gyro_cal[1] for y in df['gy']]
df['gz_detrend'] = [z - gyro_cal[2] for z in df['gz']]

#%% Linear Detrend (Accel + Gyro)
...
# https://www.kaggle.com/code/arunreddy1090/how-to-use-remove-trend-information-fm-timeseries

#%% Soft / Hard Metal Detrend

# mag_cal = [ (max(df['mx']) + min(df['mx'])) / 2, (max(df['my']) + min(df['my'])) / 2, (max(df['mz']) + min(df['mz'])) / 2]
mag_cal = [mean(df['mx']), mean(df['my']), mean(df['mz'])]

df['mx_detrend'] = [x - mag_cal[0] for x in df['mx']]
df['my_detrend'] = [y - mag_cal[1] for y in df['my']]
df['mz_detrend'] = [z - mag_cal[2] for z in df['mz']]


#%% Calibrate Plots (Accel/Gyro)

plt.figure(0)
plt.plot(x_range,df['ax_detrend']) 
plt.plot(x_range,df['ay_detrend']) 
plt.plot(x_range, df['az_detrend']) 
plt.title('Acceleration vs. Time')
plt.xlabel("Time (s)")
plt.ylabel("Acceleration (m/s^2)")
plt.legend(['X','Y','Z'])
...
plt.figure(0)
plt.plot(x_range,df['gx_detrend']) 
plt.plot(x_range,df['gy_detrend']) 
plt.plot(x_range, df['gz_detrend']) 
plt.title('Angular Velocity vs. Time')
plt.xlabel("Time (s)")
plt.ylabel("Velocity (deg/s)")
plt.legend(['X','Y','Z'])
...
#%% Calibrate Plots - Magnometer


plt.figure(0)
plt.plot(x_range,df['mx_detrend']) 
plt.plot(x_range,df['my_detrend']) 
plt.plot(x_range, df['mz_detrend']) 
plt.title('Field Strength vs. Time')
plt.xlabel("Time (s)")
plt.ylabel("Field Strength (μT)")
plt.legend(['X','Y','Z'])
...
fig, ax = plt.subplots(1, 1)
ax.set_aspect(1)
ax.scatter(df['mx_detrend'], df['my_detrend'], color='r')
ax.scatter(df['my_detrend'], df['mz_detrend'], color='g')
ax.scatter(df['mz_detrend'], df['mx_detrend'], color='b')
plt.show()

#%%
fig = plt.figure()
ax = fig.add_subplot(projection='3d')
ax.scatter(df['mx_detrend'],df['my_detrend'],df['mz_detrend'],s=2)
plt.show()
